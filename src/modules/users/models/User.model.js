    ...
    companyId: {
        ...
        validate: {
            validator: function(v) {
                return isValidCompanyId(v);
            },
            message: 'Company ID must be a valid ObjectId or start with "comp_"'
        }
    },
...
// ✅ FIX: تمت إعادة كتابة الـ hook ده بدون next() خالص (sync middleware).
// في mongoose، لو الدالة معلنة بدون باراميتر next، Kareem بينفذها sync ويكفي إنك
// تعمل throw عشان توقف العملية — ده بيتفادى مشكلة "next is not a function"
// الناتجة عن تعارض إصدارات kareem/mongoose بشكل كامل.
userSchema.pre('validate', function() {
    // ✅ تطبيع companyId
    if (this.companyId) {
        this.companyId = normalizeCompanyId(this.companyId);
    }
    ...
    if (this.companyId && !isValidCompanyId(this.companyId)) {
        throw new Error('Company ID must be a valid ObjectId or start with "comp_"');
    }
});
...
userSchema.pre('findOneAndUpdate', function() {
    // ✅ تطبيع companyId إذا كان موجوداً في الـ update
    const update = this.getUpdate();
    if (update && update.companyId) {
        update.companyId = normalizeCompanyId(update.companyId);
    }
    if (update && update.$set && update.$set.companyId) {
        update.$set.companyId = normalizeCompanyId(update.$set.companyId);
    }
...
// ✅ معالج أخطاء الحفظ — لازم يكون بالظبط 3 باراميترات (error, doc, next) عشان
// Kareem/Mongoose يتعرفوا عليه كـ error-handling middleware، وإلا هيتنفذ على كل
// عملية حفظ ناجحة كمان (وهيحط الـ document نفسه في مكان error بالغلط).
...
// ✅ نفس المبدأ هنا: لازم 3 باراميترات بالظبط عشان يتعرف عليه كـ error handler
...
userSchema.statics.getStats = async function(companyId) {
    ...
    if (companyId) {
        match.companyId = normalizeCompanyId(companyId);
    }
